...
pats_corr <- pats_weather_data |> 
  mutate(wins_per_month = as.numeric(wins_per_month),
         monthly_snowfall = as.numeric(monthly_snowfall)
         ) |> 
  ungroup() |> 
  select(where(is.numeric)) |> 
  select(monthly_mean_temp, wind_per_month, monthly_snowfall, avg_score, qbelo_post, 
         wins_per_month, monthly_mean_rain, opponent_score) |> 
  na.omit()
...
ggsave("2_figures and tables/correlation_plot.png", corr_plot)

#Checking relationship between temperature and score
...
ggsave("2_figures and tables/avg_score_scatterplot.png", score_scatterplot)

#Checking relationship between temperature and wins
...
ggsave("2_figures and tables/wins_scatterplot.png", wins_scatterplot)
